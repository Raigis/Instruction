...
2. Если нужно удалить изменения полностью используются команды:
    ...
    * `git reset --hard` - возвращает файл к состоянию на момент последнего коммита *(или коммита, на который указывает **HEAD**)*, оставляет неотслеживаемое не тронутыми.
...
* **HEAD** - указатель текущего местоположения.
...
* **Название ветки** - указывает на актуальный для данной ветки [коммит](#commit).
